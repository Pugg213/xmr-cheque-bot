...
class Settings(BaseSettings):
    ...
    app_mode: str = Field(
        default="both",
        description="Application mode: bot|monitor|both"
    )
    max_active_cheques_per_user: int = Field(
        default=10,
        description="Maximum number of active cheques per user"
    )
    cheque_ttl_seconds: int = Field(
        default=3600,
        description="Cheque time-to-live in seconds"
    )
    confirmations_final: int = Field(
        default=6,
        description="Number of confirmations for final status"
    )
    monitor_interval_sec: int = Field(
        default=30,
        description="Payment monitor polling interval in seconds"
    )
