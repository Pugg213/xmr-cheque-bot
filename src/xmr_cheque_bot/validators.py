...
def validate_view_key(view_key: str) -> None:
    ...
    if len(view_key) != 64:
        raise ValidationError(
            "view_key_length",
            f"View key must be exactly 64 hex characters, got {len(view_key)}"
        )
    
    if not _VIEW_KEY_RE.match(view_key):
        raise ValidationError(
            "view_key_format",
            "View key must contain only hexadecimal characters (0-9, a-f, A-F)"
        )
...
def validate_amount_rub(amount: int | str, min_amount: int = 100, max_amount: int = 1000000) -> int:
    ...
    if amount < min_amount:
        raise ValidationError(
            "amount_too_small",
            f"Amount too small: minimum is {min_amount} RUB, got {amount}"
        )
    
    if amount > max_amount:
        raise ValidationError(
            "amount_too_large",
            f"Amount too large: maximum is {max_amount} RUB, got {amount}"
        )
...
def validate_wallet_filename(filename: str | None) -> None:
    ...
    if "/" in filename or "\\" in filename:
        raise ValidationError(
            "wallet_filename_path_separator",
            "Wallet filename cannot contain path separators"
        )
    
    if not _WALLET_NAME_RE.match(filename):
        raise ValidationError(
            "wallet_filename_invalid",
            "Wallet filename contains invalid characters"
        )
